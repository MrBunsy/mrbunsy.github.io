...
function polygon(state,name,x,y,mass,hard,sticky,zindex,colour,thick,gametype,elastic,hollow)
{
var testarray=testnameinarray(name)
if(testarray == false)
{//if this entity isn't already in the world array, add it.....
...
//game types.  0=border, 1=ball, 2=hole, 3=not part of game
this.gametype=gametype
//how elastic the collisions are eg: 1=fully, 0.5=partially
this.elastic=elastic
//enter our object into the world array
wholeworld.push(this)
...
DrawLine(this.x[i],this.y[i],this.x[i2],this.y[i2],this.zindex,this.colour,this.thick,this.name+'class')
...
//....the worldarray already contains something by this name, so update it
wholeworld[testarray].name=name;wholeworld[testarray].x=x;wholeworld[testarray].y=y;wholeworld[testarray].mass=mass;wholeworld[testarray].div=name+'div';wholeworld[testarray].type=type;
wholeworld[testarray].vx=0;wholeworld[testarray].vy=0;wholeworld[testarray].fx=0;wholeworld[testarray].fy=0;
wholeworld[testarray].state=state
wholeworld[testarray].sticky=sticky;
wholeworld[testarray].shape=3
wholeworld[testarray].hard=hard
wholeworld[testarray].gametype=gametype
wholeworld[testarray].elastic=elastic
wholeworld[testarray].zindex=zindex;
wholeworld[testarray].colour=colour;
wholeworld[testarray].thick=thick;
wholeworld[testarray].linelength=new Array();
wholeworld[testarray].angle=new Array();
wholeworld[testarray].hollow=hollow;
var minx=wholeworld[testarray].x[0];var maxx=wholeworld[testarray].x[0];var miny=wholeworld[testarray].y[0];var maxy=wholeworld[testarray].y[0];
var dx;var dy;

for(var i=0;i<wholeworld[testarray].x.length;i++)
{var i2=i+1
if(i2==wholeworld[testarray].x.length){i2=0;}
DrawLine(wholeworld[testarray].x[i],wholeworld[testarray].y[i],wholeworld[testarray].x[i2],wholeworld[testarray].y[i2],wholeworld[testarray].zindex,wholeworld[testarray].colour,wholeworld[testarray].thick,wholeworld[testarray].name+'class')
dx=wholeworld[testarray].x[i2]-wholeworld[testarray].x[i];
dy=wholeworld[testarray].y[i2]-wholeworld[testarray].y[i];
wholeworld[testarray].angle[i]=getangle(dx,dy)
wholeworld[testarray].linelength[i]=(wholeworld[testarray].x[i2]-wholeworld[testarray].x[i])*(wholeworld[testarray].x[i2]-wholeworld[testarray].x[i])+(wholeworld[testarray].y[i2]-wholeworld[testarray].y[i])*(wholeworld[testarray].y[i2]-wholeworld[testarray].y[i])
if(wholeworld[testarray].x<minx){minx=wholeworld[testarray].x}
if(wholeworld[testarray].x>maxx){maxx=wholeworld[testarray].x}
if(wholeworld[testarray].y<miny){miny=wholeworld[testarray].y}
if(wholeworld[testarray].y>maxy){maxy=wholeworld[testarray].y}
}
wholeworld[testarray].maxx=maxx;wholeworld[testarray].minx=minx;wholeworld[testarray].maxy=maxy;wholeworld[testarray].miny=miny;
wholeworld[testarray].updatediv();
}
//functions
this.kill=kill
...
if(inpoly(oldXs[i]+Math.cos(avgangle)*radius,oldYs[i]+Math.sin(avgangle)*radius,oldXs,oldYs)!==hollow)
{avgangle+=Math.PI}

diffangle2=Math.abs(avgangle-lineangle2)+Math.abs(avgangle-lineangle1);

//alert(Math.round(todegrees(diffangle2)))

switch(hollow)
{
case true:
if(diffangle2>Math.PI){
