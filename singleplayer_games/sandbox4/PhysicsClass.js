...
var PhysicsClass = function(gravity, friction) {
	...
	this._maxSpringForce=200000;
	
	//impulse less than this will count as resting contact
	this._impulseForContact=0;
	...
	this._explosionArray=new Array();
	...
	this.save = function() {
		...
		saveObject.explosionArray=cloneObject(thisObject._explosionArray,3);
	...
	this.load = function(jsonText) {
		...
		thisObject._gravity=loadObject.gravity.slice(0);
		thisObject._friction=loadObject.friction;
		...
		//build all the explosions
		
		//redundance for old saves
		if(typeof(loadObject.explosionArray) == "undefined"){
			loadObject.explosionArray=new Array();
		}
		
		for(var i=0;i<loadObject.explosionArray.length;i++){
			var splode=loadObject.explosionArray[i];
			
			var tempSplode=thisObject.makeExplosion(splode.pos,splode.implodeTime,splode.explodeTime,splode.implodeRadius,splode.explodeRadius,splode.implodeStrength,splode.explodeStrength);
			//tempSplode.stage=splode.stage;
			//tempSplode.time=splode.time;
			tempSplode.reset(splode.time,splode.waveDistance,splode.stage);
		}
	...
	this._getExplosionForce=function(moveMe, atPos, atV){
		
		var force=[0,0,0];
		
		for(var i=0;i<thisObject._explosionArray.length;i++){
			
			var splosion=thisObject._explosionArray[i];
			
			switch(splosion.stage){
				case 0://imploding
					
					//alert([Math.dsqrd2(atPos,splosion.pos),splosion.implodeRadius2 )
					if(Math.dsqrd2(atPos,splosion.pos) < splosion.implodeRadius2){
						
						//alert(moveMe.shape)
						
						//close enough to implode - constant force inwards
						var angle=Math.atan2(splosion.pos[1]-atPos[1] , splosion.pos[0]-atPos[0]);
						force[0]+=splosion.implodeStrength*Math.cos(angle);
						force[1]+=splosion.implodeStrength*Math.sin(angle);
					}
					break;
				case 1://exploding
				/*
				 * old style-field filling whole radius
					//linearly increasing force near explosion, then constant, then linearly decreasing
					
					var distance2 =Math.dsqrd2(atPos,splosion.pos);
					
					if( distance2 < splosion.explodeRadius2){
						//close enough
						
						var angle=Math.atan2(atPos[1]-splosion.pos[1] , atPos[0]-splosion.pos[0]);
						
						
						var distance=Math.sqrt(distance2);
						
						if(distance < splosion.explodeRadius*splosion.explodeRatio){
							//close to explode
							var splodeSize=splosion.explodeStrength*(distance/(splosion.explodeRadius*splosion.explodeRatio));
							
							force[0]+=splodeSize*Math.cos(angle);
							force[1]+=splodeSize*Math.sin(angle);
						}
						else if(distance > splosion.explodeRadius-splosion.explodeRadius*splosion.explodeRatio){
							//far away
							
							var splodeSize=splosion.explodeStrength*(  splosion.explodeRadius- distance)/(splosion.explodeRadius*splosion.explodeRatio);
							
							force[0]+=splodeSize*Math.cos(angle);
							force[1]+=splodeSize*Math.sin(angle);
						}
						else{
							//in between - constant force
							force[0]+=splosion.explodeStrength*Math.cos(angle);
							force[1]+=splosion.explodeStrength*Math.sin(angle);
						}
					}
					*/
					
					//new style, wave going out
					
					var distance =Math.sqrt(Math.dsqrd2(atPos,splosion.pos));
					var min=splosion.waveDistance-splosion.waveWidth;
					var mid1=splosion.waveDistance-splosion.waveWidth/2;
					var mid2=splosion.waveDistance+splosion.waveWidth/2;
					var max=splosion.waveDistance+splosion.waveWidth;
					...
					//far side of wave
					if(mid2 < distance && distance < max){
						
						var splodeSize=splosion.explodeStrength*(  max - distance)/(splosion.waveWidth/2);
						var angle=Math.atan2(atPos[1]-splosion.pos[1] , atPos[0]-splosion.pos[0]);
						force[0]+=splodeSize*Math.cos(angle);
						force[1]+=splodeSize*Math.sin(angle);
					}
					...
					//middle of wave
					if(mid1 <= distance && distance <= mid2){
						
						var splodeSize=splosion.explodeStrength;
						var angle=Math.atan2(atPos[1]-splosion.pos[1] , atPos[0]-splosion.pos[0]);
						force[0]+=splodeSize*Math.cos(angle);
						force[1]+=splodeSize*Math.sin(angle);
					}
					...
					//inside side of wave
					if(min < distance && distance < mid1){
						var splodeSize=splosion.explodeStrength*(  distance - min)/(splosion.waveWidth/2);
						var angle=Math.atan2(atPos[1]-splosion.pos[1] , atPos[0]-splosion.pos[0]);
						force[0]+=splodeSize*Math.cos(angle);
						force[1]+=splodeSize*Math.sin(angle);
					}
	...
	//if shape is in contact with other shapes, make the force normal to the surface of contact zero
	this._getContactForce=function(moveMe, atPos, atV, currentForce){
		var force=[0,0,0];
		
		for(var i=0;i<moveMe.contactWith.length;i++){
			var contactStuff=thisObject._findContactNormal(moveMe,moveMe.contactWith[i]);
			if(contactStuff===false){
				i--;
				continue;
			}
			var normal=contactStuff.contactNormal;
			var point=contactStuff.contactPoint;
			//console.log(normal);
			var forceAlongNormal=Math.dotProduct([currentForce[0],currentForce[1]],normal);
			
			force[0]=-forceAlongNormal*normal[0];
			force[1]=-forceAlongNormal*normal[1];
			
			force[2]=-currentForce[2];
			
			if(forceAlongNormal < moveMe.mass*thisObject._gravity[1])
			{
				thisObject.breakContactBetween(moveMe,moveMe.contactWith[i]);
				i--;
			}
		}
		return force;
	...
	this._findContactNormal=function(moveMe,shape2){
		//find the surface normal for two resting contact shapes - move them closer together and use collision detection to work this out?
		//bodge, but will hopefully work
		
		var collide;
		var tempPos=moveMe.pos.slice(0);
		var i=0;
		var angle=Math.atan2(shape2.pos[1]-moveMe.pos[1] ,shape2.pos[0]-moveMe.pos[0]);
		
		//slowly move the shapes closer together until we get a collision and thus a contact normal from the collision detection 
		do{
			//tempPos=[tempPos[0]+Math.cos(angle) , tempPos[1]+Math.sin(angle), tempPos[2]];
			tempPos[0]+=Math.cos(angle);
			tempPos[1]+=Math.sin(angle);
			collide=thisObject._detect.checkForCollision(tempPos, moveMe.d, moveMe.hollow, tempPos[2], moveMe.shape, shape2.pos, shape2.d, shape2.hollow, shape2.pos[2], shape2.shape);
			
			i++;
		}
		//infinite loop protection
		while(!collide.overlap && i<50)
		
		if(i==50){
			//try moving outwards
			tempPos=moveMe.pos.slice(0);
			
			//console.log("Error: shapes don't collide!");
			i=0;
			do{
			//tempPos=[tempPos[0]+Math.cos(angle) , tempPos[1]+Math.sin(angle), tempPos[2]];
			tempPos[0]-=Math.cos(angle);
		...
		if(i==50){
			//too far away
			thisObject.breakContactBetween(moveMe,shape2);
			return false;
		}
	...
	this._getAcceleration = function(moveMe, atPos, atV) {
		...
		var explosion = thisObject._getExplosionForce(moveMe, atPos, atV);
		
		
		force[0] += gravity[0] + friction[0] + spring[0] + charge[0] + explosion[0];
		force[1] += gravity[1] + friction[1] + spring[1] + charge[1] + explosion[1];
		...
		//for surface contact
		var contact = thisObject._getContactForce(moveMe, atPos, atV, force.slice(0));
	...
	this.advance = function(dt) {
		...
		for (var i = 0; i < thisObject._canMoveArray.length; i++) {
			...
			if (!collide.collision) {
				//no collision, move shape and give it new velocity
				moveMe.pos = newPosAndV.pos.slice(0);
				moveMe.v = newPosAndV.v.slice(0);
			}
			else{
				//collision, do not move this shape and dish out new velocities
				//var newVs = thisObject._response.collideTwoShapes(moveMe, newPosAndV.pos, newPosAndV.v, collide.otherShape, collide.contactPoint, collide.contactNormal)
				var newVs = thisObject._response.collideTwoShapes(moveMe, newPosAndV.pos, moveMe.v, collide.otherShape, collide.contactPoint, collide.contactNormal)
				
				//alert(newVs.impulse);
				
				
				//KE before
				//console.log(Math.modulus(moveMe.v)+" , "+Math.modulus(newVs.v1));
				
				
				moveMe.v=newVs.v1.slice(0);
				
				if(Math.abs(newVs.impulse)<thisObject._impulseForContact){
					//console.log(newVs.impulse);
					//thisObject.makeContactBetween(moveMe,collide.otherShape);
				}else{
					thisObject.breakContactBetween(moveMe,collide.otherShape);
				}
		...
		for(var i=0;i<thisObject._explosionArray.length;i++){
			thisObject._explosionArray[i].advance(dt);
		}
	}
	...
	this.makeContactBetween=function(shape1,shape2){
		
		
		
		if(inArray(shape2, shape1.contactWith))
			return;
		shape1.makeContactWith(shape2);
		shape2.makeContactWith(shape1);
		
		thisObject._setRelativeVelocityZero(shape1,shape2);
	}
	
	this._setRelativeVelocityZero=function(shape1,shape2){
		//set relative velocities to 0
		var contactStuff=thisObject._findContactNormal(shape1,shape2)
		var normal=contactStuff.contactNormal;
		
		var v2=shape2.canMove ? shape2.v : [0,0];
		//find speed of shape 2 in normal direction
		//set speed of shape 1 to that speed by finding its speed that directiopn and getting rid of the difference
		
		var shape1NormalSpeed=Math.dotProduct([shape1.v[0],shape1.v[1]] , normal);
		var shape2NormalSpeed=Math.dotProduct([v2[0],v2[1]] , normal);
		
		var diff=shape1NormalSpeed-shape2NormalSpeed;
		
		shape1.v[0]-=diff*normal[0];
		shape1.v[1]-=diff*normal[1];
		
		//idea for friction - find velocity perpendicular to normal and use mew on that?
	}
	
	
	
	
	this.breakContactBetween=function(shape1,shape2){
		shape1.breakContactWith(shape2);
		shape2.breakContactWith(shape1);
	}
	...
	*/
	...
	this.makeExplosion=function(pos,implodeTime,explodeTime,implodeRadius,explodeRadius,implodeStrength,explodeStrength){
		return new thisObject.Explosion(pos, implodeTime,explodeTime,implodeRadius,explodeRadius,implodeStrength,explodeStrength);
	}
	
	this.Explosion=function(pos, implodeTime,explodeTime,implodeRadius,explodeRadius,implodeStrength,explodeStrength){
		
		var thisSplosion=this;
		...
		this.implodeTime=implodeTime;
		this.implodeRadius=implodeRadius;
		this.implodeRadius2=implodeRadius*implodeRadius;
		this.implodeStrength=implodeStrength;
		
		this.explodeTime=explodeTime;
		this.explodeRadius=explodeRadius;
		this.explodeRadius2=explodeRadius*explodeRadius;
		this.explodeRatio=0.2;
		this.explodeStrength=explodeStrength;
		
		this.waveSpeed=explodeRadius/explodeTime;
		this.waveWidth=explodeRadius*this.explodeRatio;
		
		//stages: 0=imploding, 1=exploding
		this.stage=0;
		
		this.time=0;
		this.waveDistance=0;
		
		this.reset=function(t,distance,stage){
			this.time=t;
			this.waveDistance=distance;
			this.stage=stage;
		}
		
		this.advance=function(dt){
			thisSplosion.time+=dt;
			switch(thisSplosion.stage){
				case 0://imploding
					if(thisSplosion.time>thisSplosion.implodeTime){
						this.stage=1;
						//move to exploding
					}
				case 1://exploding
					thisSplosion.waveDistance+=dt*thisSplosion.waveSpeed;
					if(thisSplosion.time > thisSplosion.implodeTime+thisSplosion.explodeTime){
						//end of exploding, kill this off
						thisSplosion.alive=false;
						removeFromArray(thisSplosion, thisObject._explosionArray);
					}
			}
		}
		
		
		thisObject._explosionArray.push(this);
	}
	...
	this.Circle = function(radius, position, angle, velocity, mass, force, hard, elasticity, hollow, canMove, visible, colour, gameType) {
		...
		//shapes in contact with
		this.contactWith=new Array();
		
		this.makeContactWith=function(withMe){
			thisShape.contactWith.push(withMe);
		}
		
		this.breakContactWith=function(withMe){
			removeFromArray(withMe, thisShape.contactWith);
		}
		/*
