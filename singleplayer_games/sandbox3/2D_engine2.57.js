...
else if(wholeworld[us].type==0 && wholeworld[us].shape==3 && wholeworld[them].type==1)
...
else if(wholeworld[us].type==0 && wholeworld[them].type==0)
...
theirnewvx= wholeworld[them].vx - PCi*PCnormal[0]/wholeworld[them].mass
theirnewvy= wholeworld[them].vy - PCi*PCnormal[1]/wholeworld[them].mass
theirnewvw = wholeworld[them].vw - PCi*(PCRap[0]*PCnormal[1] - PCRap[1]*PCnormal[0])/PCbI

if(debug){togglepause()
...
/*
if(newvw>100){newvw=100}
if(newvw<-100){newvw=-100}

if(theirnewvw>100){theirnewvw=100}
if(theirnewvw<-100){theirnewvw=-100}

//dalert(newvx+" "+newvy+" "+newvw+" "+theirnewvx+" "+theirnewvy+" "+theirnewvw)
dalert(newvx+" "+theirnewvw)
*/
...
function dalert(saythis)
{
if(debug){debug=confirm(saythis);}
}
...
function NormAndPointPolyAndPoly(poly1,poly2)
{
PCpoint=CLr;
/*
if(polycorner)
{var whichpoly=polynumber;}
else{var whichpoly=Math.ceil(Math.random()*2);}

alert(polycorner)

switch(whichpoly)
{case 1:var tempangle=getangleofpolyline(poly2,polyline2)+Math.PI/2;break;
case 2:var tempangle=getangleofpolyline(poly1,polyline)+Math.PI/2;break;}
*/
...
switch(polynumber)
{
case 1:var tempangle=getangleofpolyline(poly2,polyline)+Math.PI/2;break;
case 2:var tempangle=getangleofpolyline(poly1,polyline)+Math.PI/2;break;
}
}else{
...
PCnormal=[Math.cos(tempangle),Math.sin(tempangle)];

}

function NormAndPointPolyAndCircle(poly,circle)
{

//PCpoint=CLr;//Circle Line intersection

if(polycorner)//corner of polygon collided with circle - take tangent to circle as collision surface
{
//angle between collision point and centre of circle
var tempangle=Math.atan2(CLr[1]-circle.y,CLr[0]-circle.x);
}
else//side of polygon collided with circle - take edge of poly as collision surface
{
...
PCpoint=[circle.x+Math.cos(tempangle)*circle.width/2 , circle.y+Math.sin(tempangle)*circle.width/2];
...
function getangleofpolyline(them,k)
{
...
else if(s2==3 && s1!==3){var polycollide=polyandshape(x2,y2,w2,h2,r2,a2,f2,x1,y1,w1,h1,s1)}
else if(s2==3 && s1==3)
{
var polycollide=polyandpoly(x1,y1,w1,h1,r1,a1,f1,x2,y2,w2,h2,r2,a2,f2)
}
...
var polynumber=0;//which polygon? the first or second.

function polyandpoly(poly1X,poly1Y,poly1Rs,poly1As,poly1bigR,poly1angle,poly1filled,poly2X,poly2Y,poly2Rs,poly2As,poly2bigR,poly2angle,poly2filled)
...
if(poly1filled)//testing corners of poly2 inside poly1
{
...
CLr=[polyx1,polyy1];
polynumber=2;polycorner=true;return true;}
...
CLr=[polyx1,polyy1];
polynumber=1;polycorner=true;return true;}
}//end of loop
}//end of poly2filled
...
var ya2=poly1Y+poly1Rs[k2]*Math.sin(poly1As[k2]+poly1angle);
    ...
    for(var p=0;p<poly2Rs.length;p++)
		...
		{polyline=k;polyline2=p;return true;}
...
function findpolyline(x,y,polyX,polyY,polyRs,polyAs,polyangle)
{
var polyx1=polyX+polyRs[0]*Math.cos(polyAs[0]+polyangle);
var polyy1=polyY+polyRs[0]*Math.sin(polyAs[0]+polyangle);
var polyx2=polyX+polyRs[1]*Math.cos(polyAs[1]+polyangle);
...
if(testshort<shortest){shortest=testshort;temppolyline=k;}
}
return temppolyline;
}
...
{polyline=k;polycorner=true;CLr=[polyx1,polyy1];return true;}
}
}
polycorner=false;
...
*/
//distance between point and each end of the polygon's line, minus the polygon's line.
polyline=findpolyline(theircentrex,theircentrey,polyX,polyY,polyRs,polyAs,polyangle);
/*
polyline=0;
var shortest=dsqrd(polyx1,polyy1,theircentrex,theircentrey)+dsqrd(polyx2,polyy2,theircentrex,theircentrey)-dsqrd(polyx2,polyy2,polyx1,polyy1)
...
if(testx1<=x && x<= testx2 && testy1<=y && y<=testy2){CLr=[x,y];return true;}
//plus
var y=(-(-2*b)+Math.sqrt(discrim))/2
if(testx1<=x && x<= testx2 && testy1<=y && y<=testy2){CLr=[x,y];return true;}
}
