...
Response.collidewithpoly=function(us,them)//isn't 'us' only ever going to be something which can move?  need to check this.
  {
	...
	if(Shapes.wholeworld[us].canMove && Shapes.wholeworld[us].shape!==3 && Shapes.wholeworld[them].type==1 && Shapes.wholeworld[them].shape==3)
	  {
		Response.staticPolyAndNonPoly(Shapes.wholeworld[us],Shapes.wholeworld[them]);
		}
	else if(Shapes.wholeworld[them].canMove && Shapes.wholeworld[them].shape!==3 && Shapes.wholeworld[us].type==1 && Shapes.wholeworld[us].shape==3)
	  {
		Response.staticPolyAndNonPoly(Shapes.wholeworld[them],Shapes.wholeworld[us]);
		}
	else if(Shapes.wholeworld[us].canMove && Shapes.wholeworld[us].shape==3 && Shapes.wholeworld[them].type==1)
		...
		switch(Shapes.wholeworld[them].shape)//set PCnormalx, PCnormaly, PCpointx, PCpointy
		  {
			...
			break;
			case 1://square
			  Response.NormAndPointPolyAndSquare(Shapes.wholeworld[us],Shapes.wholeworld[them],us,newx,newy,Shapes.wholeworld[them].pos[0],Shapes.wholeworld[them].pos[1],newangle)
			break;
			case 1://polygon
			  Response.NormAndPointPolyAndPoly(Shapes.wholeworld[us],Shapes.wholeworld[them],us,newx,newy,Shapes.wholeworld[them].pos[0],Shapes.wholeworld[them].pos[1],newangle)
			break;
			}
			...
			PCRap=[PCpoint[0]-Shapes.wholeworld[us].pos[0],PCpoint[1]-Shapes.wholeworld[us].pos[1]];
			PCRbp=[PCpoint[0]-Shapes.wholeworld[them].pos[0],PCpoint[1]-Shapes.wholeworld[them].pos[1]];
			...
			PCi= ( (-1)*(1+Shapes.wholeworld[us].elastic)*(PCVa1[0]*PCnormal[0]+PCVa1[1]*PCnormal[1]) )/( (1/Shapes.wholeworld[us].mass)  + rapnsqrd/PCaI);

			Response.newvx= Shapes.wholeworld[us].v[0] + PCi*PCnormal[0]/Shapes.wholeworld[us].mass
			Response.newvy= Shapes.wholeworld[us].v[1] + PCi*PCnormal[1]/Shapes.wholeworld[us].mass
			Response.newvw = Shapes.wholeworld[us].v[2] + PCi*(PCRap[0]*PCnormal[1] - PCRap[1]*PCnormal[0])/PCaI
		  ...
		  {//corner of poly lies INSIDE square
			...
			if(oldpointX >= oldsquareX+square.dimensions[0]/2 && oldpointY <= oldsquareY+square.dimensions[1]/2 && oldpointY >= oldsquareY-square.dimensions[1]/2)
			  ...
			  {//LEFT hand side
				PCnormalx=-1;
				PCnormaly=0;
				PCpointx=oldsquareX-square.dimensions[0]/2;
				}
			  ...
			  {//ABOVE
				PCnormalx=0;
				PCnormaly=-1;
				PCpointy=oldsquareY+square.dimensions[1]/2;
				}
			else// if(oldpointY >= oldsquareY+square.dimensions[1]/2 && oldpointX >= oldsquareX-square.dimensions[0]/2 && oldpointX <= oldsquareX+square.dimensions[0]/2)
			  {//BELOW
				PCnormalx=0;
	...
	if(Detect.polycorner)//corner of polygon collided with circle - take tangent to circle as collision surface
	  {
		...
		var tempangle=Math.atan2(CLr[1]-circle.pos[1],CLr[0]-circle.pos[0]);
		}
	else
	  {
