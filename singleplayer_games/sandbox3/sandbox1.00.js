...
Sandbox.rotatestuff=function()
  ...
  if(Sandbox.mouserotate!==false)
    ...
    if(document.getElementById('rotatecollisions').checked==false || Detect.checkforanycollide2b(Shapes.wholeworld[Sandbox.mouserotate],testangle)===false)
      ...
      prerotatex=UI.mouseX;
      prerotatey=UI.mouseY;
      if(Sandbox.paused)
			  {
				Render.redrawworld();
				}
  ...
  for (i=0;i<Shapes.wholeworld.length;i++)
    ...
    if(Shapes.wholeworld[i].canMove && Shapes.wholeworld[i].alive)
      ...
      if(Sandbox.followmouse==i)
  			{
        newx=UI.mouseX/Render.viewports[0].zoom+Render.viewports[0].x+UI.mousedragdifference[0];newy=UI.mouseY/Render.viewports[0].zoom+Render.viewports[0].y+UI.mousedragdifference[1];
        dragoldx=Shapes.wholeworld[i].pos[0];
        dragoldy=Shapes.wholeworld[i].pos[1];
  			}
...
Sandbox.mousegonedown=function(event,divstuff)
  ...
  here=Detect.checkforanycollide3(UI.mouseX/Render.viewports[0].zoom+Render.viewports[0].x,UI.mouseY/Render.viewports[0].zoom+Render.viewports[0].y,mousesize,mousesize,0)//see what mouse cursor is colliding with
  /*break;
  default:
  here=divstuff
  break}
...
document.onmousemove = UI.getMouseXY;

//set up the graphdiv to collect the scrollwheel info
if (window.addEventListener)
  {
	document.getElementById("mainworldcanvas").addEventListener('DOMMouseScroll', UI.wheel, false);
	}
if(General.IE)
  {
	window.onmousewheel = document.onmousewheel = UI.wheel;
	}
document.getElementById("mainworldcanvas").onmousewheel = UI.wheel;
