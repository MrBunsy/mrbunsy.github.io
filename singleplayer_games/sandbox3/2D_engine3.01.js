		...
		if(Render.drawcounter%Render.viewports[r].sync==0 && Render.viewports[r].enabled)
		  {
			...
			for (i4=0;i4<Shapes.wholeworld.length;i4++)
			  {
				if(Shapes.wholeworld[i4].visible)
				  {
...
Render.drawCircle=function(vp)
  {
	General.dalert(this.pos)
	var tempx=this.pos[0];
	var tempy=this.pos[1]
	var tempr=this.dimensions
	var tempfunc=function()
		{
		
		Render.viewports[vp].canvas.moveTo((tempx-Render.viewports[vp].x+tempr)*Render.viewports[vp].zoom, (tempy-Render.viewports[vp].y)*Render.viewports[vp].zoom);
		Render.viewports[vp].canvas.arc((tempx-Render.viewports[vp].x)*Render.viewports[vp].zoom,(tempy-Render.viewports[vp].y)*Render.viewports[vp].zoom,tempr*Render.viewports[vp].zoom/2,0,Math.PI*2,true);  // draw circle
		}
...
Render.drawRectangle=function(vp)
  {
	var tempfunc=function()
		{
		//top left
		Render.viewports[vp].canvas.moveTo((this.pos[0]-this.dimensions[0]/2-Render.viewports[vp].x)*Render.viewports[vp].zoom, (this.pos[1]-this.dimensions[1]/2-Render.viewports[vp].y)*Render.viewports[vp].zoom);
		//top right
		Render.viewports[vp].canvas.lineTo((this.pos[0]-this.dimensions[0]/2+Render.viewports[vp].x)*Render.viewports[vp].zoom, (this.pos[1]-this.dimensions[1]/2-Render.viewports[vp].y)*Render.viewports[vp].zoom);
		//bottom right
		Render.viewports[vp].canvas.lineTo((this.pos[0]-this.dimensions[0]/2+Render.viewports[vp].x)*Render.viewports[vp].zoom, (this.pos[1]+this.dimensions[1]/2-Render.viewports[vp].y)*Render.viewports[vp].zoom);
		//bottom left
		Render.viewports[vp].canvas.lineTo((this.pos[0]-this.dimensions[0]/2-Render.viewports[vp].x)*Render.viewports[vp].zoom, (this.pos[1]+this.dimensions[1]/2-Render.viewports[vp].y)*Render.viewports[vp].zoom);
...
Detect.checkforanycollide3=function(x,y,w,h,s,r,a,f)
  ...
  for(var z=0;z<Shapes.wholeworld.length;z++)
    ...
    if(Shapes.wholeworld[z].alive && Shapes.wholeworld[z].hard)// && wholeworld[z].hard)
      {
      var testcollide=Detect.checkcollide(x,y,w,h,s,Shapes.wholeworld[z].x,Shapes.wholeworld[z].y,Shapes.wholeworld[z].width,Shapes.wholeworld[z].height,Shapes.wholeworld[z].shape,r,Shapes.wholeworld[z].bigR,a,Shapes.wholeworld[z].angle,f,Shapes.wholeworld[z].filled)
      if(testcollide == true)
  			{
  			return z;
  			}
...
Detect.checkcollide=function(p1,d1,f1,a1,s1,p2,d2,f2,a2,a2,s2)
  ...
  //shapes,0=circle, 1= rectangle,  2=square/rectangle (obselete, I think), but collide as circle, 3 = polygon, 4= line
  ...
  else if(s1==3 || s2==3)//involves a polygon
    {
  	...
  	if(x1+d1 > x2-d2[0]/2 && x1-d1 < (x2+d2[0]/2) && y1-d1/2 < (y2+d2[1]/2) && y1+d1/2 > y2-d2[1]/2)
		  {
			return true;
			}
  	}
