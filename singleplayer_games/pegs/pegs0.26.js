// Pegs Version 0.26
// Copyright Luke Wallin 2008 (www.lukewallin.co.uk)
// Tested in IE 8, FF 3, Opera 9.5, Safari 3 (windows beta)
// Should work in IE 6+, FF 1.5+, Opera 9+, Safari 3+ and any browser which supports the canvas.
// First game to use my new technique of putting all javascript in one global variable.
// Add the following script to place this game in a page:
/*

<!--[if IE]><script type="text/javascript" src="excanvas_gradstuff_compressed.js"></script><![endif]-->
<div id="pegsdiv" style="width:500;height:500;"></div>
<script src="pegs0.26.js" type="text/javascript"></script>

*/
// Change the style of pegsdiv to change the size of the game.
// It will work in any dimensions, but is designed to be square.
...
document.getElementById('pegsdiv').innerHTML+='<span id="pegsspan"></span><span style="display:none">this makes it work in IE for some bizzare reason</span><canvas id="pegscanvas" width="'+(document.getElementById('pegsdiv').style.width)+'" height="'+(document.getElementById('pegsdiv').style.height)+'"></canvas>'
...
pegs.load=function()
  ...
  this.div.onmousemove=pegs.getMouse;
	...
	if (this.canvastag.getContext)
	  {
		this.canvas = document.getElementById('pegscanvas').getContext('2d');
//		alert(this.canvas)
		}
	...
	//make menus
	this.mainMenu=new this.menu("<center>Welcome to Pegs<br><br><span style=\"font-size:"+(pegs.height*0.1)+"px;\" onClick=\"pegs.startGame();\">Play</span><br><br><span id='pegsdifficultspan' onClick=\"pegs.changedifficult();\">Difficulty: Medium</span><br><br><span onClick='pegs.helpMenu.open();'>Help</span><br><br><br><span style=\"font-size:"+(pegs.height*0.03)+"px;\"><a style='color:black;' href='http://www.lukewallin.co.uk/games/pegs'>&copy; Luke Wallin 2008</a></span></center>");
	this.helpMenu=new this.menu("<span style=\"font-size:"+(pegs.height*0.05)+"px;\"><center><b>How to Play</b></center></span><span style=\"font-size:"+(pegs.height*0.035)+"px;\"><br>The aim of Pegs is to logically deduce the colours and locations of the hidden pegs.<br><br>After placing 4 pegs on the board click <b>Ready</b> to see how many were right.  A black marker means one was the right colour and in the right place, a white marker means one peg was the right colour but in the wrong place.<br><br>You have only 8 attempts to get all the pegs right.<br><br>On <b>easy</b> there are only six colours and each peg will be a different colour.  On <b>medium</b> there can be repetitions.  On <b>hard</b> there are eight colours and on <b>mastermind</b> there are eight colour and repetitions.<br><br></span><span style='font-size:"+(pegs.height*0.04)+"px;\' onClick='if(pegs.inGame) {pegs.closeMenus();pegs.reDraw(false);} else{ pegs.mainMenu.open();}'>Back</span>");
	this.mainMenu.open();
	...
	switch(pegs.difficult)
	  {
			...
			for(var i=0;i<pegs.pegs.length;i++)
			  {
				...
				while(pegs.inarray(randomcolour,pegs.pegs))
				  {
					randomcolour=Math.floor(Math.random()*5.99);
					}
				pegs.pegs[i]=randomcolour
				}
		break;
		case 1://medium (6 colours, repetitions)
		  pegs.pegs=new Array(4);
			pegs.colours=6;
			for(var i=0;i<pegs.pegs.length;i++)
			  {
		...
		break;
		case 2://hard (8 colours, but NO repetitions)
			pegs.pegs=new Array(4);
			pegs.colours=8;
			for(var i=0;i<pegs.pegs.length;i++)
			  {
				var randomcolour=Math.floor(Math.random()*7.99);
				//no repetiton - check to see if new colour is in pegs array.
				while(pegs.inarray(randomcolour,pegs.pegs))
				  {
					randomcolour=Math.floor(Math.random()*7.99);
					}
				pegs.pegs[i]=randomcolour
				}		
		break;
		case 3://mastermind (8 colours, repetitions)
		  pegs.pegs=new Array(4);
			pegs.colours=8;
			for(var i=0;i<pegs.pegs.length;i++)
			  {
...
pegs.reDraw=function(firsttime)
  {
	...
	//cover up canvas with blank div to stop IE being able to highlight it.
	new this.text('',1,1,0,0,0.1)
