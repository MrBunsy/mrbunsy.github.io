...
MWBG.load=function()
  {
	...
	//add span for text
	document.getElementById('MWBGdiv').innerHTML+="<span id='textSpan'></span><div id='MWBGeditordiv' style='display:none;position:absolute;margin-left:"+(MWBG.width)+";width:"+(MWBG.width*0.3)+";height:"+(MWBG.height*0.6)+";border:solid black 1px;'><h3>Editor</h3><span onClick='MWBG.openMainMenu();'>Back to Menu</span><br><br><span onClick='MWBG.changePlacing(0)' id='MWBGplace0'>Place Land</span><br><span onClick='MWBG.changePlacing(1)' id='MWBGplace1'>Place Hill</span><br><span onClick='MWBG.changePlacing(2)' id='MWBGplace2'>Place Blockage</span><br><span onClick='MWBG.changePlacing(3)' id='MWBGplace3'>Place Water</span><br><span onClick='MWBG.changePlacing(4)' id='MWBGplace4'>Place Spawn</span><br><br><span onClick='MWBG.saveMap()'>Save Map</span></div>"
...
MWBG.highlighttext=function(thisdiv)
  ...
  document.getElementById(thisdiv).select();
	...
	/*
MWBG.arrayToText=function(thisArray)
  {
	var arrayText='[';
	...
	while(key < thisArray.length)
	  {
		
		if(typeof thisArray[key] !=='object')
		  {
			//not another array
			arrayText+=thisArray[key];
			key++;
			}
		else
		  {
...
//update type of cell.
MWBG.makeCellType=function(thistype)
  {
	this.type=thistype;
	switch(thistype)
	  {
	...
	if(MWBG.makingMap)
	  {
		...
		MWBG.mapCells[thisCell].make(MWBG.placing);
		MWBG.drawBoard();
	  }
	}
