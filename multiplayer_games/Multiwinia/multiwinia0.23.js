...
MWBG.battle=function()
  {
	...
	setTimeout(MWBG.battlePart2,5000);
	}
...
MWBG.canFire=function(thisPeice)
  {
	...
	if(MWBG.peices[thisPeice].diceRoll===3 || MWBG.peices[thisPeice].diceRoll===4 || (MWBG.hasThisType(peiceInCell,1) && (MWBG.peices[thisPeice].diceRoll===2 || MWBG.peices[thisPeice].diceRoll===5) ) )
	  {
		//without office, die is 4 or 5, with office die is 3,4,5 or 6 - can fire laser!
		return true;
		}
	return false;
	}
...
MWBG.laser=function(from,to,speed,colour)
  {
	...
	this.physics=new Shapes.circle(true,true,from,0,1,1,0,[speed*Math.cos(angle),speed*Math.sin(angle),0],[0,0,0],false,true,true,0,this.arrayPos)
...
MWBG.drawDie=function(number,peice)
  {
	...
	MWBG.drawBox(tempX-tempSize/2 , tempY-tempSize/2 , tempSize , tempSize , "rgb(255,255,255)" ,"rgb(0,0,0)",1)
	...
	for(var i=0;i<MWBG.lasers.length;i++)
	  {
		if(MWBG.lasers[i].alive)
  		{
  		MWBG.lasers[i].pos=MWBG.lasers[i].physics.pos[i];
			MWBG.canvas[1].save();
