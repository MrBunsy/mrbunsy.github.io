...
MWBG.load=function()
  {
	//are peices drawn with random numbers on dice?
	MWBG.randomDice=false;
...
MWBG.battle=function()
  {
	...
	if(battleHappens)
  	{
		//start dice randomlu changing
		MWBG.randomDice=true;
		MWBG.loop=setInterval(MWBG.drawPeices,100);
		
		//stop dice randomly changing
		setTimeout(function(){clearInterval(MWBG.loop);MWBG.randomDice=false;MWBG.drawPeices();},2000);
		
  	MWBG.drawPeices();
  	setTimeout(MWBG.battlePart2,4000);
  	}
	else
	  {
		MWBG.endOfBattle();
	...
	if(MWBG.selected.length>0)
	  {
		...
		if(MWBG.slowBrowser)
  		...
  		{
			//instead of searching whole board for cells we can move to - search a grid
			var startCol=MWBG.mapCells[MWBG.selectedFrom].col-smallestMove-1
			startCol=(startCol < 0 ? 0: startCol);
			
			var endCol=MWBG.mapCells[MWBG.selectedFrom].col+smallestMove+1
			endCol=(endCol > MWBG.cols ? MWBG.cols: endCol);
			
			var startRow=MWBG.mapCells[MWBG.selectedFrom].row-smallestMove-1
			startRow=(startRow < 0 ? 0: startRow);
			
			var endRow=MWBG.mapCells[MWBG.selectedFrom].row+smallestMove+1
			endRow=(endRow > MWBG.Rows ? MWBG.Rows: endRow);
		...
		for(var i2=0;i2<cellCount;i2++)
		  {
			...
			if(MWBG.peices[MWBG.mapCells[i].peices[i2]].diceRoll!==false)
			  {
				//drawing a random number or the real dice roll?
				thisNumber=MWBG.randomDice? Math.floor(Math.random()*6) : MWBG.peices[MWBG.mapCells[i].peices[i2]].diceRoll
				//draw a die
				MWBG.drawDie(thisNumber,MWBG.mapCells[i].peices[i2])
				}
			else
			  {
...
//do two cells border?
MWBG.cellsNear=function(cell1,cell2)
  {
	for(var i=0;i<6;i++)
	  {
		//loop through cells nearby to cell1 to see if cell2 is one of them.
		if(cell2 == MWBG.getCellInThisDir(cell1,i))
		  {
			return true;
			}		
		}
...
MWBG.getWeight=function(cell,water)
  {
	//only if cell is on board and got a distance
	switch(MWBG.mapCells[cell].type)
	  {
		case 0://normal land
			weight=1;
		break;
...
MWBG.cellDistance=function(cell1,cell2,water)
  {
	...
	if(MWBG.cellsNear(cell1,cell2))
	  {
		//these two border
		return MWBG.getWeight(cell2,water);
		}
