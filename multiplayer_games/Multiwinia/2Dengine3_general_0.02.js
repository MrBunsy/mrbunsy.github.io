...
General.removeFromArray=function(needle,haystack)
  {
	for (var k=0;k<haystack.length;k++)
	  {
		if(haystack[k] === needle)
...
General.addtoifnotin=function(needle,haystack)
  {
	var testin=0;
...
General.getHighest=function(haystack)
  {
	var highest=haystack[0];
	
	for(var z=0;z<haystack.length;z++)
	  {
		if(haystack[z]>highest)
		  {
			highest=haystack[z];
			}
		}
...
General.getLowest=function(haystack)
  {
	var lowest=haystack[0];
	
	for(var z=0;z<haystack.length;z++)
	  {
		if(haystack[z]<lowest)
		  {
			lowest=haystack[z];
			}
		}
