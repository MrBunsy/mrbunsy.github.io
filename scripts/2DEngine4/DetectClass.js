...
//contactPoint is relative to the first shape
//contact normal is the normal on the surface of the second shape, must be a unit vector!

var DetectClass = function(){
	var thisObject = this;
	
	//store functions in 2d array to avoid messy lookups
	this._collidingFunctions = new Array(5);
	for (var i = 0; i < 5; i++) 
	{
		this._collidingFunctions[i] = new Array(5);
	}
	
	this._checkCircleAndCircle = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkCircleAndBorder = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkBorderAndCircle = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
		return thisObject._checkCircleAndBorder(p2, d2, h2, a2, s2, p1, d1, h1, a1, s1)
	}
	
	this._checkPolyAndBorder = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkPolyAndCircle = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkCircleAndPoly = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkPolyAndPoly = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2)
	{
		...
		*/
		...
		//this._linesIntersect = function(sx1, sy1, sx2, sy2, px1, py1, px2, py2)
		/*
		
		*/
	}
	
	this._checkRodAndBorder = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkRodAndCircle = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkCircleAndRod = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
	...
	this._checkRodAndPoly=function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2) {
		var x1=p1[0] + Math.cos(a1) * d1;
		var y1=p1[1] + Math.sin(a1) * d1;
		var x2=p1[0] + Math.cos(a1 + Math.PI) * d1;
		var y2=p1[1] + Math.sin(a1 + Math.PI) * d1;
	...
	this._checkPolyAndRod=function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2) {
		var x1=p2[0] + Math.cos(a2) * d2;
		var y1=p2[1] + Math.sin(a2) * d2;
		var x2=p2[0] + Math.cos(a2 + Math.PI) * d2;
		var y2=p2[1] + Math.sin(a2 + Math.PI) * d2;
	...
	this._checkRodAndRod=function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2) {
	...
	this._collidingFunctions[0][0] = this._checkCircleAndCircle;
	this._collidingFunctions[0][2] = this._checkCircleAndBorder;
	this._collidingFunctions[0][3] = this._checkCircleAndPoly;
	this._collidingFunctions[0][4] = this._checkCircleAndRod;
	
	this._collidingFunctions[3][0] = this._checkPolyAndCircle;
	this._collidingFunctions[3][2] = this._checkPolyAndBorder;
	this._collidingFunctions[3][3] = this._checkPolyAndPoly;
	this._collidingFunctions[3][4] = this._checkPolyAndRod;
	
	this._collidingFunctions[4][0] = this._checkRodAndCircle;
	this._collidingFunctions[4][2] = this._checkRodAndBorder;
	this._collidingFunctions[4][3] = this._checkRodAndPoly;
	this._collidingFunctions[4][4] = this._checkRodAndRod;
	...
	this.checkForCollision = function(p1, d1, h1, a1, s1, p2, d2, h2, a2, s2){
		return this._collidingFunctions[s1][s2](p1, d1, h1, a1, s1, p2, d2, h2, a2, s2);
	}
	
	
	//used by poly and circle checks
	this._checkPointsInsideCircle = function(polyPos, polyangle, polyRs, polyAs, circlePos, circleR){
	...
	///used by poly and circle checks
	this._checkPolyLinesAndCircle = function(polyPos, polyangle, polyRs, polyAs, circlePos, circleR){
		var polyX = polyPos[0];
		var polyY = polyPos[1];
		...
		for (var k = 0; k < polyRs.length; k++) 
		{
			...
			var polyCircle = this._checkCircleAndLineCollide(polyx1, polyx2, polyy1, polyy2, circlePos[0], circlePos[1], circleR);
	...
	this._inpoly = function(x, y, polyX, polyY, polyRs, polyAs, polyangle){
		...
		for (var p = 0; p < polyRs.length; p++) 
		{
			...
			if (this._linesIntersect(x, y, x2, y, polyX + polyRs[p] * Math.cos(polyAs[p] + polyangle), polyY + polyRs[p] * Math.sin(polyAs[p] + polyangle), polyX + polyRs[p2] * Math.cos(polyAs[p2] + polyangle), polyY + polyRs[p2] * Math.sin(polyAs[p2] + polyangle)).overlap) 
			{
				crosslines++;
			}
		}
	...
	this._findPolyLineNormal = function(x, y, polyX, polyY, polyRs, polyAs, polyangle){
	...
	this._checkCircleAndLineCollide = function(x1, x2, y1, y2, a, b, r){
		//r=radius
		//a=circle centre x
		//b=circle centre y
