...
SpiderGen.load=function()
  ...
  if (SpiderGen.canvastag.getContext)
	...
	General.debug=true;
	SpiderGen.runAlone=true;
	
	new SpiderGen.spider(25,1,0,[250,0],1);
	
	//new SpiderGen.spider(25,1,1,[500,250],-1);
	
	//new SpiderGen.spider(25,1,1,[10,250],1);
        ...
        case 0://drop down then swing back and forth
    		switch(SpiderGen.spiders[i].stage)
					...
					switch(SpiderGen.spiders[i].stage)
      		{
      				...
      				SpiderGen.spiders[i].legs[SpiderGen.spiders[i].movingLeg].footPos[1]+=Math.abs(SpiderGen.spiders[i].speed)*2*( SpiderGen.spiders[i].legs[SpiderGen.spiders[i].movingLeg].stepStep < 9 ? -1 : 1 )
      				SpiderGen.spiders[i].legs[SpiderGen.spiders[i].movingLeg].kneePos[1]+=Math.abs(SpiderGen.spiders[i].speed)*( SpiderGen.spiders[i].legs[SpiderGen.spiders[i].movingLeg].stepStep < 9 ? -1 : 1 )
    					...
    					if(SpiderGen.spiders[i].legs[SpiderGen.spiders[i].movingLeg].stepStep > 15)
    					  {
    						...
    						SpiderGen.spiders[i].movingLeg%=4;
								...
								if(Math.random()<0.05 && SpiderGen.spiders[i].movingLeg==0)
  							  {
							...
							if(SpiderGen.spiders[i].pos[1] > SpiderGen.spiders[i].startPos[1])
							  {
								...
								dy=SpiderGen.spiders[i].startPos[1]-SpiderGen.spiders[i].pos[1];
				  ...
				  //draw top segments of body
					...
					//bodge for facing other way:
					tempSize=-SpiderGen.spiders[i].size*SpiderGen.spiders[i].facing;
					
					var leftHead=function()
					{
					//left of head
					SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0] - tempSize/2 , SpiderGen.spiders[i].pos[1]);
					//top of head
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] , SpiderGen.spiders[i].pos[1] - tempSize/4);
					//bottom of head
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] , SpiderGen.spiders[i].pos[1] + tempSize/4);
					//left of head
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] - tempSize/2 , SpiderGen.spiders[i].pos[1]);
					}
					
					var rightHead=function()
					{
					//top of head
					SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0] , SpiderGen.spiders[i].pos[1] - tempSize/4);
					//top right of head
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*0.4 , SpiderGen.spiders[i].pos[1] - tempSize*0.1);
					...
					{
					//top right of head
					SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0] + tempSize*0.4 , SpiderGen.spiders[i].pos[1] - tempSize*0.1);
					//top of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/4);
					//top mid of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/8);
					//top right of head
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*0.4 , SpiderGen.spiders[i].pos[1] - tempSize*0.1);
					}
					...
					{
					//top of body
					SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/4);
					//top right of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.55 , SpiderGen.spiders[i].pos[1] - tempSize*0.6);
					//top mid right of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.725 , SpiderGen.spiders[i].pos[1] - tempSize*0.3);
					//mid top of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/8);
					//top of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/4);
					...
					{
					//mid top of body
					SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0] + tempSize*1.2 , SpiderGen.spiders[i].pos[1] - tempSize*3/8);
					//top mid right of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.725 , SpiderGen.spiders[i].pos[1] - tempSize*0.3);
					//right of body
					SpiderGen.canvas.lineTo(SpiderGen.spiders[i].pos[0] + tempSize*1.9 , SpiderGen.spiders[i].pos[1]);
					...
					for(var i2=0;i2<SpiderGen.spiders[i].legs.length;i2++)
					  {
						SpiderGen.canvas.moveTo(SpiderGen.spiders[i].pos[0]+SpiderGen.spiders[i].facing*SpiderGen.spiders[i].size*7/24 - SpiderGen.spiders[i].facing*SpiderGen.spiders[i].size*i2/5 , SpiderGen.spiders[i].pos[1])
					...
					*/
					...
					if((SpiderGen.spiders[i].pos[0]<-SpiderGen.spiders[i].size*2 &&  SpiderGen.spiders[i].facing==-1) || (SpiderGen.spiders[i].pos[0] > SpiderGen.width+SpiderGen.spiders[i].size*2  && SpiderGen.spiders[i].facing==1))
					  {
						//walked off screen.
						SpiderGen.spiders[i].alive=false;
						SpiderGen.nextSpider();
...
SpiderGen.nextSpider=function()
  {
	...
	switch(SpiderGen.spiders.length%3)
  	{
  	case 0:
  	new SpiderGen.spider(25,1,0,[250,0],1);
  	break;
  	...
  	break;
  	case 2:
  	new SpiderGen.spider(25,1,1,[50,250],1);
  	break;
  	}
	}
...
SpiderGen.spider=function(size,LBratio,doThis,startPos,facing)
  {
	this.alive=true;
	...
	switch(doThis)
	  {
		...
		case 1://darwinia walking spider
			...
			this.body=new Shapes.circle(false,true,startPos,0,size,50,0,[0,0,0],[0,0,0],false,true,false,3,SpiderGen.spiders.length);
			
		  //animation step (for jumping)
			this.step=0;
			
		  //pixels per second
		  this.speed=SpiderGen.width*0.001*(size/50)*this.facing;
		  ...
		  this.legs=new Array(4)
			...
			for(var i=0;i<this.legs.length;i++)
			  {
				tempFoot=[startPos[0]+facing*size*1.5-facing*i*size , startPos[1]+size]
				tempKnee=[startPos[0]+facing*size*1.5/2-facing*i*size/2 , startPos[1]-size]
