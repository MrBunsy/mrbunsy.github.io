...
TreeObject.leaf=function(start,size,steps)
  {
	this.type=1;//leaf
	this.start=start;
	this.size=size;// /10;
		...
		if(TreeObject.trees[t].alive)
  		{
			...
			switch(TreeObject.trees[t].branches[TreeObject.trees[t].branches.length-1].type)
			  {
    						...
    						break;
								case 1://card
								  var tempcard=new Cards.card(Math.round(Math.random()*3),Math.round(Math.random()*12+1),true)
									...
									tempcard.draw((TreeObject.trees[t].branches[i].start[0]-TreeObject.trees[t].branches[i].size/2)/Cards.width , (TreeObject.trees[t].branches[i].start[1]-TreeObject.trees[t].branches[i].size*1.5/2)/Cards.height , (TreeObject.trees[t].branches[i].size)/Cards.width , 0 , true)
								break;
    						}
						break;
  					}
    				...
    				if(TreeObject.trees[t].branches[b].alive)
    				  {
    					...
    					if(drawingLeaves)
  						  {
  							TreeObject.trees[t].branches.push(new TreeObject.leaf(TreeObject.trees[t].branches[b].finish , TreeObject.trees[t].size*TreeObject.trees[t].leafSize , TreeObject.trees[t].steps))
  							}
							else
  							{
