...
TreeObject.load=function()
  ...
  TreeObject.canvas.lineCap="butt";
	...
	//[ [204,153,0] , [50,200,0] ]
  new TreeObject.tree(250,450,4,3,100,3,[3,3],0.8, [[255,100,0,0.8],[255,100,0,0.5]] , 0.5 , 0 , 1 ,"rgba(0,255,0,0.3)");
	}
...
TreeObject.tree=function(x,y,iterations,branchings,size,steps,thick,ratio,colour,angle,leaves,leafSize,leafColour)
  {
	this.root=[x,y];
	...
	this.ratio=ratio
	if(colour[0].length==3)
	  {
		//no alpha
		colour[0].push(1)
		}
	if(colour[1].length==3)
	  {
		//no alpha
		colour[1].push(1)
		}
	//array:  [ start , finish  ]' start+finish: [ R , G , B (, A ) ]'
	this.colour=colour;
	this.dcolour=[ (colour[1][0]-colour[0][0])/iterations , (colour[1][1]-colour[0][1])/iterations , (colour[1][2]-colour[0][2])/iterations , (colour[1][3]-colour[0][3])/iterations];
	...
	if(typeof leafColour=='undefined')
	  {
		leafColour="rgb(0,200,0)";
		}
...
TreeObject.leaf=function(start,size,steps,leafType)
  {
	this.type=1;//leaf
	this.start=start;
	this.size=size;// /10;
	this.steps=steps;
	//alert([this.size,this.steps])
			...
			switch(TreeObject.trees[tree].atStage)
			  {
				case 0://drawing branches
				  TreeObject.canvas.strokeStyle="rgba("+Math.round(TreeObject.trees[tree].colour[0][0]+TreeObject.trees[tree].dcolour[0]*TreeObject.trees[tree].iteration)+","+Math.round(TreeObject.trees[tree].colour[0][1]+TreeObject.trees[tree].dcolour[1]*TreeObject.trees[tree].iteration)+","+Math.round(TreeObject.trees[tree].colour[0][2]+TreeObject.trees[tree].dcolour[2]*TreeObject.trees[tree].iteration)+","+(TreeObject.trees[tree].colour[0][3]+TreeObject.trees[tree].dcolour[3]*TreeObject.trees[tree].iteration)+")";
				break;
  			...
  			if(TreeObject.trees[tree].branches[branch].alive)
    			{
          		...
          		TreeObject.canvas.lineTo(TreeObject.trees[tree].branches[branch].start[0]+TreeObject.trees[tree].branches[branch].dx*TreeObject.trees[tree].branches[branch].progress , TreeObject.trees[tree].branches[branch].start[1]+TreeObject.trees[tree].branches[branch].dy*TreeObject.trees[tree].branches[branch].progress)
						...
						case 1://leaf
      					...
      					case 0://circle
    							TreeObject.trees[tree].branches[branch].progress++
									//alert([TreeObject.trees[tree].branches[branch].start,TreeObject.trees[tree].branches[branch].ds,TreeObject.trees[tree].branches[branch].progress])
									//alert([TreeObject.trees[tree].branches[branch].start[0]+TreeObject.trees[tree].branches[branch].ds*TreeObject.trees[tree].branches[branch].progress,TreeObject.trees[tree].branches[branch].start[1]])
      					...
      					for(var b2=0;b2<TreeObject.trees[tree].branchings;b2++)
      					  {
      						...
      						TreeObject.trees[tree].branches.push(new TreeObject.branch(TreeObject.trees[tree].branches[b].finish , [TreeObject.trees[tree].branches[b].finish[0]+Math.cos(newangle)*TreeObject.trees[tree].size*(Math.random()*0.75+0.5) , TreeObject.trees[tree].branches[b].finish[1]+Math.sin(newangle)*TreeObject.trees[tree].size*(Math.random()*0.75+0.5)] , TreeObject.trees[tree].steps))
