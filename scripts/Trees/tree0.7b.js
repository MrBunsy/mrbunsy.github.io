		...
		if(TreeObject.trees[tree].alive)
  		{
			...
			//alert(TreeObject.trees[tree].branches.length)
			...
			//opera.postError('got past chosing branch type')
    						...
    						break;
								case 1://card
								  //var tempcard=new Cards.card(Math.round(Math.random()*3),Math.round(Math.random()*12+1),true)
									//opera.postError('drawing leaf1')
									TreeObject.trees[tree].branches[branch].progress++
									tempsize=TreeObject.trees[tree].branches[branch].ds*TreeObject.trees[tree].branches[branch].progress;
									//(x,y,width,where,full)
									//alert(TreeObject.trees[tree].branches[branch].size)
									//alert([(TreeObject.trees[tree].branches[branch].start[0]-TreeObject.trees[tree].branches[branch].size/2)/Cards.width , (TreeObject.trees[tree].branches[branch].start[1]-TreeObject.trees[tree].branches[branch].size*1.5/2)/Cards.height , (TreeObject.trees[tree].branches[branch].size)/Cards.width , 0 , true])
									//alert([TreeObject.trees[tree].branches[branch].card.suit , TreeObject.trees[tree].branches[branch].card.number])
									//if(!Cards.IE || TreeObject.trees[tree].branches[branch].progress==TreeObject.trees[tree].branches[branch].steps)
									if(false)
  									{
										//opera.postError('drawing leaf2')
										//opera.postError([(TreeObject.trees[tree].branches[branch].start[0]-tempsize/2)/Cards.width , (TreeObject.trees[tree].branches[branch].start[1]-tempsize*1.5/2)/Cards.height , (tempsize)/Cards.width , 0 , true])
										if(tempsize>0)
  										{
